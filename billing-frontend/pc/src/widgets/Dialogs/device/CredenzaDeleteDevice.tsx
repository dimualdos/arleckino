...
type Props = {
  ...
  toggle: () => void
  onChangeDeleteDevices: () => void
}

export const DialogDeleteDevice = ({ open, onChange, toggle, onChangeDeleteDevices }: Props) => {
  return (
    <Credenza
      open={open}
      ...
      <CredenzaContent className="block p-0">
        <div className="relative flex items-start space-x-3.5 border-b p-6 md:space-x-0 md:border-0 md:pb-2">
          ...
          <div className="relative space-y-2 md:space-y-4">
            <p className="text-xl font-bold">Удаление устройства</p>
            <div className="space-y-4 md:text-grey-extra-hard">
              <p>Устройство и его данные будут безвозвратно удалены. Продолжить?</p>
            </div>
          </div>
        </div>
