  ...
  ({ maxHeight = Number.MAX_SAFE_INTEGER, minHeight = 52, className, onChange, value, ...props }: AutosizeTextAreaProps, ref: React.Ref<AutosizeTextAreaRef>) => {
    ...
    return (
      <textarea
        ...
        ref={textAreaRef}
        className={cn(
          "flex min-h-10 w-full rounded-md border outline-none border-grey-soft bg-background px-3 py-3 ring-accent-muted ring-offset-background transition-all duration-300 placeholder:text-grey focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50",
          className,
        )}
        onChange={(e) => {
